...
let rec cStmt stmt (varEnv: varEnv) (funEnv: funEnv) : instr list =
    ...
    | Switch(e, cases) ->
        let endLabel = newLabel () //label for the end of the switch
        let exprCode = cExpr e varEnv funEnv // Compile the expression

        let comparisonCode =
            cases
            |> List.mapi (fun index (value, stmt) ->
                let label = newLabel ()
                let compare = [ DUP; CSTI value; EQ; IFNZRO label ]
                let stmtCode = cStmt stmt varEnv funEnv @ [ GOTO endLabel ] // Code for the case statement with jump to end
                (compare, stmtCode, label))
        // Combine all generated code
        let allCode =
            exprCode
            @ (comparisonCode
               |> List.collect (fun (compare, stmtCode, label) -> compare @ [ Label label ] @ stmtCode))
            @ // Flatten the lists of code
            [ Label endLabel ] // Add the end label
...
and cExpr (e: expr) (varEnv: varEnv) (funEnv: funEnv) : instr list =
    ...
    | PreInc(acc) ->
        let addr = cAccess acc varEnv funEnv
        addr @ [ DUP ] @ [ LDI ] @ [ CSTI 1; ADD ] @ [ STI ]
    | PreDec(acc) ->
        let addr = cAccess acc varEnv funEnv
        addr @ [ DUP ] @ [ LDI ] @ [ CSTI 1; SUB ] @ [ STI ]
    | Prim1(ope, e1) ->
        cExpr e1 varEnv funEnv
        @ (match ope with
           | "!" -> [ NOT ]
