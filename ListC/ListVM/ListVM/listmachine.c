...
void mark(word *block)
{
  ...
  {
    word *referencedBlock = getReferencedBlock(block, i);
...
{
  printf("sweeping ...\n");
  word *current = heap, *prev = NULL;

  while (current < afterHeap)
  {
    if (isMarked(current))
      unmarkBlock(current);
    else
    {
      ...
      word *next = current + blockSize(current) + 1;
      if (next < afterHeap && !isMarked(next))
      {
        word currentSize = blockSize(current);
        currentSize += blockSize(next) + 1;
        setBlockSize(current, currentSize);
        current = next;
      }
