...
//ii: the fun v -> 2 * v doubles the computed length, which demonstrates tail-recursion, so:
// 1 + 0 = 1 -> 2
// 1 + 2 = 3 -> 6
// 1 + 6 = 7 -> 14
...
let rec leni xs a =
    ...
    | [] -> a
    | _ :: xr -> leni xr (a + 1)
//Both are tail-recursive, so they avoid building up a call stack, they make up for constant time.
//Acc store the value that has been computed so far, Cont tell functions what to do after - they carry the result forward
...
//ii: the revc xs (fun v -> v @ v) would take the reversed list and append it to itself.
...
let rec prodcOpti xs k =
    match xs with
    | [] -> k 1
    | 0 :: _ -> k 0 // If a zero is found return 0
    | x :: xr -> prodcOpti xr (fun v -> k (x * v))
...
let ex10 =
    Letfun("foo", "y", CstI 3, Var "foo") (* Function foo used as variable. *)
