...
let e2: expr = Let(["z", CstI 17], Prim("+", Let(["z", CstI 22], Prim("*", CstI 100, Var "z")), Var "z"));;
...
let rec eval e (env : (string * int) list) : int =
    ...
    | Var x -> lookup env x 
    | Let(l, exp) -> 
    let newEnv = List.fold (fun acc (s, e) -> (s, eval e acc) :: acc) env l
    in eval exp newEnv
...
let e1ev: int = eval e1 [("z", 200)];;

let e2ev = eval e2 [("z", 100)];;
